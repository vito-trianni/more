...
while [ $# -gt 0 ]; do
    ...
    if [ "${OPTION}" == "clean" ]; then
        rm -rf ${ARGOSINSTALLDIR}/build/simulator/*
    fi
    # build type
...
mkdir -p ${ARGOSINSTALLDIR}/build/simulator; cd ${ARGOSINSTALLDIR}/build/simulator || { echo "Error: directory ${ARGOSINSTALLDIR}/build/simulator does not exists, and the attemp to create it failed: permissions trouble?" 1>&2; exit 1; }
