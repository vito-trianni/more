   ...
   /****************************************/
	...
	void CPovrayRenderVisitorDraw::Visit(CCylinderEntity& c_entity) {
		...
		if(!IgnoreEntity(c_entity.GetId())) {
			...
			std::string strColorOrTexture = "pigment { color Yellow }";
			strColorOrTexture = GetBoxOrCylinderColorOrTexture(c_entity.GetId(), strColorOrTexture);
			...
			TLedEntityList& tLeds =  cLEDEquippedEntity.GetAllLeds();

			for(UInt32 i = 0; i < tLeds.size(); ++i) {

				CLedEntity* pcTmp = tLeds[i];

				// Output
				m_cOutputFileStream << "//" << c_entity.GetId() << "\n"

				// Output light declaration
				<< "#declare light_source_body_" << c_entity.GetId() << " = object { sphere { <0,0,0>, 0.02 } texture { pigment {color rgb <" << GetPovrayColorValue(pcTmp->GetColor().GetRed()) << "," << GetPovrayColorValue(pcTmp->GetColor().GetGreen()) <<  "," << GetPovrayColorValue(pcTmp->GetColor().GetBlue()) << ">} finish {ambient .8 diffuse .6}} }"<< "\n"

				// Now write the actual light source
				<< "light_source{  <" << pcTmp->GetPosition().GetX() << "," << pcTmp->GetPosition().GetY() <<  "," << pcTmp->GetPosition().GetZ() << "> color rgb <" << GetPovrayColorValue(pcTmp->GetColor().GetRed()) << "," << GetPovrayColorValue(pcTmp->GetColor().GetGreen()) <<  "," << GetPovrayColorValue(pcTmp->GetColor().GetBlue())<< "> looks_like {light_source_body_" << c_entity.GetId()<< "}     fade_distance 0.2 fade_power 5 shadowless }" << "\n\n"
