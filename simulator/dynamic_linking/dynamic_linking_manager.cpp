...
std::map<std::string, CCI_Controller_Maker *, std::less<std::string> >& GetControllerFactory() {
   static std::map<std::string, CCI_Controller_Maker *, std::less<std::string> > mapControllerFactory;
   return mapControllerFactory;
}

/* The map of loop functions */
std::map<std::string, CLoopFunctions_Maker *, std::less<std::string> >& GetLoopFunctionFactory() {
   static std::map<std::string, CLoopFunctions_Maker *, std::less<std::string> > mapLoopFunctionFactory;
   return mapLoopFunctionFactory;
}
...
namespace argos {
   ...
   void CDynamicLinkingManager::LoadLoopFunctions(const std::string& str_library_name,
      ...
      if (str_library_name != "") {
         try {
            ...
            if(GetLoopFunctionFactory().find(str_label) == GetLoopFunctionFactory().end()) {
            ...
            pcLoopFunctions = GetLoopFunctionFactory()[str_label]();
         }
         catch(CARGoSException& ex) {
   ...
   CCI_Controller* CDynamicLinkingManager::NewController(CEntity& c_entity,
      ...
      try {
         ...
         if(GetControllerFactory().find(strControllerName) == GetControllerFactory().end()) {
            THROW_ARGOSEXCEPTION("Unknown controller type \"" << strControllerName << "\"");
         }

         // Create a controller with the found name
         CCI_Controller* pcController = GetControllerFactory()[strControllerName]();
